...
import { precreateProject, uploadProjectDocs } from "@/services/project-service";
      ...
      <div className="flex-1 overflow-auto px-6 pb-6">
        <div className="max-w-3xl mx-auto mt-8">
          ...
          <div className="flex items-center justify-between mb-8 px-4">
            <div className={`flex flex-col items-center ${currentStep === 'upload' ? 'text-primary' : 'text-gray-500'}`}>
              <div className={`w-8 h-8 rounded-full flex items-center justify-center border-2 mb-2 ${currentStep === 'upload' ? 'border-primary bg-primary text-white' : (currentStep === 'confirm' || currentStep === 'generating') ? 'border-green-500 bg-green-500 text-white' : 'border-gray-300'}`}>
                {currentStep === 'confirm' || currentStep === 'generating' ? <Check className="w-5 h-5" /> : "1"}
              </div>
              <span className="text-sm font-medium">上传文件</span>
            </div>
            <div className={`flex-1 h-0.5 mx-4 ${currentStep === 'confirm' || currentStep === 'generating' ? 'bg-green-500' : 'bg-gray-200'}`}></div>
            <div className={`flex flex-col items-center ${currentStep === 'confirm' ? 'text-primary' : 'text-gray-500'}`}>
              <div className={`w-8 h-8 rounded-full flex items-center justify-center border-2 mb-2 ${currentStep === 'confirm' ? 'border-primary bg-primary text-white' : currentStep === 'generating' ? 'border-green-500 bg-green-500 text-white' : 'border-gray-300'}`}>
                {currentStep === 'generating' ? <Check className="w-5 h-5" /> : "2"}
              </div>
              <span className="text-sm font-medium">确认信息</span>
            </div>
            <div className={`flex-1 h-0.5 mx-4 ${currentStep === 'generating' ? 'bg-green-500' : 'bg-gray-200'}`}></div>
            <div className={`flex flex-col items-center ${currentStep === 'generating' ? 'text-primary' : 'text-gray-500'}`}>
              <div className={`w-8 h-8 rounded-full flex items-center justify-center border-2 mb-2 ${currentStep === 'generating' ? 'border-primary bg-primary text-white' : 'border-gray-300'}`}>
                3
              </div>
              <span className="text-sm font-medium">生成工序</span>
            </div>
          </div>
              ...
              {currentStep === 'upload' && (
                <div className="space-y-8">
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="project-name" className="text-base">项目名称 <span className="text-red-500">*</span></Label>
                    <Input 
                      id="project-name" 
                      placeholder="请输入项目名称" 
                      value={projectName} 
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-2">
                      ...
                      <div 
                        className="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center hover:bg-gray-50 transition-colors cursor-pointer h-64 flex flex-col items-center justify-center"
                      ...
                      >
                        {!cadFile && (
                          <>
                            ...
                            <p className="font-medium text-gray-900 mb-1">点击上传CAD图纸</p>
                            <p className="text-xs text-gray-500">支持 .dwg, .dwf, .dxf 格式</p>
                          </>
                        )}
                        {cadFile && (
                          <div className="space-y-4 w-full">
                            ...
                            <p className="text-sm font-medium text-gray-900 truncate px-4">{cadFile.name}</p>
                            <Button variant="outline" size="sm" className="text-red-600 hover:text-red-700 hover:bg-red-50 border-red-200" onClick={(e) => {
                              e.stopPropagation();
                      ...
                      >
                        {!projectDoc && (
                          <>
                            ...
                            <p className="font-medium text-gray-900 mb-1">点击上传说明文档</p>
                        ...
                        {projectDoc && (
                          <div className="space-y-4 w-full">
                            ...
                            <p className="text-sm font-medium text-gray-900 truncate px-4">{projectDoc.name}</p>
                            <Button variant="outline" size="sm" className="text-red-600 hover:text-red-700 hover:bg-red-50 border-red-200" onClick={(e) => {
                              e.stopPropagation();
                  ...
                  <div className="flex justify-end gap-3 pt-4">
                    <Button variant="outline" onClick={() => navigate(-1)} className="h-10 px-6">
                      取消
                    </Button>
                    <Button 
              ...
              {currentStep === 'confirm' && (
                <div className="space-y-8">
                  ...
                  <div className="flex justify-end gap-3 pt-4">
                    ...
                    <Button onClick={handleGenerateProcess} disabled={isCreating} className="h-10 px-6">
                      生成工序
                    </Button>
                  </div>
                </div>
                ...
                <div className="space-y-8 py-8">
                  <div className="flex flex-col items-center justify-center space-y-6 text-center">
                    ...
                    <div className="space-y-2 max-w-md">
                      <h3 className="text-xl font-semibold text-gray-900">正在生成施工工序</h3>
                      <p className="text-gray-500">系统正在分析上传文件并生成施工计划，这可能需要几分钟时间，请稍候...</p>
                    </div>

                    <div className="w-full max-w-md space-y-4 text-left bg-gray-50 p-6 rounded-lg border border-gray-100">
                      <div className="flex items-center gap-3">
                        <div className="w-2 h-2 bg-green-500 rounded-full"></div>
                        <p className="text-sm font-medium text-gray-700">正在上传项目文件...</p>
                      </div>
                      <div className="flex items-center gap-3">
                        <div className="w-2 h-2 bg-blue-500 rounded-full animate-pulse"></div>
                        <p className="text-sm font-medium text-gray-700">正在解析CAD图纸结构...</p>
                      </div>
                      <div className="flex items-center gap-3">
                        <div className="w-2 h-2 bg-gray-300 rounded-full"></div>
                        <p className="text-sm text-gray-500">等待生成施工工序...</p>
                      </div>
                    </div>
                  </div>
                </div>
