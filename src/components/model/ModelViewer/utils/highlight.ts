...
export function useHighlight({
  ...
  const applyHighlightRef = useRef<((model: THREE.Object3D & { modelID: number }, attempt?: number) => void | Promise<void>) | null>(null);
  ...
  const scheduleHighlightRetry = useCallback((nextAttempt: number) => {
