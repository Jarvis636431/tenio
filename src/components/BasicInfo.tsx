...
export function BasicInfo() {
  ...
  // Check if current form values differ from default values
  const checkForChanges = () => {
    const currentValues = form.getValues();
    const changed = JSON.stringify(currentValues) !== JSON.stringify(defaultValues);
    if (changed !== hasChanges) {
      setHasChanges(changed);
    }
  };

  // Use useEffect to check for changes whenever form values change
  React.useEffect(() => {
    checkForChanges();
  }, [watchedValues]);
