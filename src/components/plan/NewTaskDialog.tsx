...
interface NewTaskDialogProps {
  ...
  onAdd: (task: Partial<PlanTask>) => void;
...
export function NewTaskDialog({
  ...
  const [pendingTask, setPendingTask] = useState<Partial<PlanTask> | null>(
    null,
  );
  ...
  // 过滤前置任务选项
  const filteredPrerequisiteTasks = existingTasks.filter(
    (task) =>
      task.task.toLowerCase().includes(prerequisiteSearch.toLowerCase()) &&
      !formData.prerequisiteTasks.includes(task.id),
  );
  const displayPrerequisiteTasks = filteredPrerequisiteTasks.slice(
    0,
    prerequisiteVisible,
  );

  // 过滤后置任务选项
  const filteredDependentTasks = existingTasks.filter(
    (task) =>
      task.task.toLowerCase().includes(dependentSearch.toLowerCase()) &&
      !formData.dependentTasks.includes(task.id),
  );
  const displayDependentTasks = filteredDependentTasks.slice(
    0,
    dependentVisible,
  );
  ...
  // 当搜索或面板重新打开时，重置可见数量
  useEffect(() => {
    setPrerequisiteVisible(100);
  }, [prerequisiteSearch, prerequisiteOpen]);
  useEffect(() => {
    setDependentVisible(100);
  }, [dependentSearch, dependentOpen]);
      ...
      await addProcess(
        {
          ...
          predecessor_processes:
            pendingTask.prerequisiteProcess?.replace(/\s+/g, "") || undefined,
          successor_processes:
            pendingTask.directDependency?.replace(/\s+/g, "") || undefined,
  ...
  const getSelectedTaskNames = (taskIds: number[]) => {
    return taskIds
      .map((id) => existingTasks.find((task) => task.id === id)?.task)
      .filter(Boolean);
  };
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-2">
                  ...
                  <Popover
                    ...
                    onOpenChange={setPrerequisiteOpen}
                  >
                        ...
                        <div className="flex flex-wrap gap-2 text-left">
                          {formData.prerequisiteTasks.length === 0 ? (
                            ...
                            getSelectedTaskNames(
                              formData.prerequisiteTasks,
                            ).map((taskName, index) => (
                              <Badge
                                key={index}
                                variant="secondary"
                                className="flex items-center gap-1 bg-category-blue-100 text-category-blue-800 border-category-blue-200 hover:bg-category-blue-200 transition-colors"
                              >
                                {taskName}
                                <button
                                  type="button"
                                  ...
                                  onClick={(e) => {
                                    ...
                                    removePrerequisiteTask(
                                      formData.prerequisiteTasks[index],
                                    );
                                  }}
                                >
                    ...
                    <PopoverContent className="w-[--radix-popover-trigger-width] p-0">
                      <Command>
                        ...
                        <div
                          ...
                          onScroll={handlePrerequisiteScroll}
                        >
                          <div className="p-1">
                              ...
                              displayPrerequisiteTasks.map((task) => (
                                ...
                                >
                                  <Check
                                    className={cn(
                                      "mr-2 h-4 w-4",
                                      formData.prerequisiteTasks.includes(
                                        task.id,
                                      )
                                        ? "opacity-100"
                                        : "opacity-0",
                  ...
                  <Popover open={dependentOpen} onOpenChange={setDependentOpen}>
                      ...
                      >
                        <div className="flex flex-wrap gap-2 text-left">
                          {formData.dependentTasks.length === 0 ? (
                            ...
                            getSelectedTaskNames(formData.dependentTasks).map(
                              (taskName, index) => (
                                <Badge
                                  key={index}
                                  variant="secondary"
                                  className="flex items-center gap-1 bg-category-green-100 text-category-green-800 border-category-green-200 hover:bg-category-green-200 transition-colors"
                                >
                                  {taskName}
                                  <button
                                    type="button"
                    ...
                    <PopoverContent className="w-[--radix-popover-trigger-width] p-0">
                      <Command>
                        ...
                        <div
                          className="max-h-72 overflow-auto"
                          onScroll={handleDependentScroll}
                        >
                          <div className="p-1">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="p-3 rounded-md bg-muted/40">
                ...
                <div className="text-base font-medium">
                  {(formData.dependentTasks?.length || 0) + 2} 道
                </div>
              </div>
              <div className="p-3 rounded-md bg-muted/40">
                <div className="text-xs text-muted-foreground">
                ...
                <div className="text-base font-medium">
                  ¥{Number(formData.totalCost || 0).toLocaleString()}
                </div>
              </div>
              <div className="p-3 rounded-md bg-muted/40">
                <div className="text-xs text-muted-foreground">工期变化</div>
                <div className="text-base font-medium">
                  +{Number(formData.duration) || 1} 天
                </div>
              </div>
              <div className="p-3 rounded-md bg-muted/40">
                <div className="text-xs text-muted-foreground">
